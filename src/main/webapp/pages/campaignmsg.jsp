    ...
    <body onload="startTimer()">
        ...
        <div id="wrapper">
            <div id="content" class="clearfix" style="z-index:0">
                ...
                <form:form action="${pageContext.request.contextPath}/SMS" commandName="campaigns" method="POST" role="form" acceptCharset="utf-8" enctype="multipart/form-data">    
                    ...
                    <div class="page-message" style="padding-left: 15px; padding-top: 15px; float: left;">
                        <div class="message" id="message">${not empty message?message:''}</div>
                                ...
                                <c:forEach var="folder" items="${folderList}" varStatus="loop">
                                    <c:if test="${not empty folder.messageTypes}" >
                                        <dt>       
                                            ...
                                            <form:hidden path="folderHasCampaignses[${count}].id.fhCid"/>
                                            ...
                                            <div style="padding:14px; padding-right: 0px;width:100%;padding-left: 0px;position:relative;left:-10px;" class="pull-left">  
                                                <div class="form-group">
                                                    <div class="col-lg-3 col-md-13 col-sm-12 col-xs-12 padd-left-zero-ipad" style="padding-left:15px; padding-right:0px;">         
                                                        ...
                                                        <form:select multiple="false" path="folderHasCampaignses[${count}].selectedEvents" class="form-control selectpicker show-tick width-101" style="height:auto;">  
                                            ...
                                            <div class="full-width overflow-auto pull-left">  
                                                ...
                                                <c:forEach var="message" items="${folder.messageTypes}" varStatus="childLoop">
                                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin: 0px;padding:0px;">    
                                                        ...
                                                        <form:hidden path="campaignMessageses[${childcounter}].messageId" />
                                                        <form:hidden id="pushNotification${childcounter}" path="campaignMessageses[${childcounter}].pushNotification"/>
                                                        ...
                                                        <div class="form-group padd-bottom-zero-ipad" style="padding:10px 14px 10px 14px; background: white;border:1px solid #2071b6;float: left;width:100%;border-radius:2px;">  
                                                            <div class="col-lg-12 col-xs-12 form-group btn_Color" style="padding-left: 0px;margin-bottom:10px;float: left;">
                                                                ...
                                                                <div id="testSMSDiv" class="col-lg-2 col-md-12 col-sm-12 col-xs-12 padding-phoneno-div">
                                                                    <div class="btn-group">
                                                                        <div class="btndrop plusCol" onclick="saveCampaign(${count},${childcounter}, 0)">Save</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div style="clear: both;"></div>
                                                            <div class="col-lg-9" style="padding-left: 5px;">
                                                                <form:input id="subject${childcounter}" path="campaignMessageses[${childcounter}].subject" cssClass="form-control" placeholder="Subject"/>
                                                            ...
                                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 for-320 padd-right-zero" id = "response${childcounter}" style="padding-left:0px;padding-top:5px;">  
                                                                ...
                                                                <c:forEach begin="0" end="${responselength}" varStatus="loopdd">
                                                                    <form:hidden path="campaignMessageses[${childcounter}].campaignMessagesResponses[${responsecounter}].campaignMessagesResponseId"/>
...
<!--                                                                            <button style="margin-top:4px; position: relative; right: -28px;" type="button" onclick="removeResponse('${childcounter}${responsecounter}', '${childcounter}')"><i class="fa fa-minus-circle"></i></button>-->
                                                                            <a href="#" class="fa fa-minus-circle" style="margin-top:4px; position: relative; right: -28px;display: none;" onclick="removeResponse('${childcounter}${responsecounter}', '${childcounter}')"></a>
                                                                        </c:if>
...
<!--                                                                            <button style="margin-top:23px;position:relative;right:-4px;" type="button" onclick="addResponse('response${childcounter}', '${childcounter}')"><i class="fa fa-plus-circle"></i></button> -->
                                                                            <a href="#" class="fa fa-plus-circle" style="margin-top:28px;position:relative;right:-4px;display: none;" onclick="addResponse('response${childcounter}', '${childcounter}')"></a>
                                                                        </c:if>
                                                                        <br clear="all" />
                                                                    </div>
                ...
                <script type="text/javascript">
                    ...
                    function hideMessage() {
                        if ($("#errorMessage").text().length > 0 && $('#errorMessage').css('display') == 'block') {
                            document.getElementById("errorMessage").style.display = "none";
                        }
                        if ($("#message").text().length > 0 && $('#message').css('display') == 'block') {
                            document.getElementById("message").style.display = "none";
                        }
                    }

                    function startTimer() {
                        window.setInterval("hideMessage()", 60000);  // 5000 milliseconds = 5 seconds
                    }
                    ...
                    function saveCampaign(parentIndex, childIndex, responseIndx) {

                        var folderId = $("#" + "folderHasCampaignses" + parentIndex + "\\.folder\\.folderId1").val();
                        // alert(folderId);
                        var fhCid = $("#folderHasCampaignses" + parentIndex + "\\.id\\.fhCid").val();
                        //alert(fhCid);
                        var selectedEvents = $("#" + "folderHasCampaignses" + parentIndex + "\\.selectedEvents").val();
                        // alert(selectedEvents);
                        var messageTypeId = $("#" + "campaignMessageses" + childIndex + "\\.messageTypeId1").val();
                        // alert(messageTypeId);
                        var isCritical = $("#" + "campaignMessageses" + childIndex + "\\.isCritical1").val();
                        // alert(isCritical);
                        var subject = $("#subject" + childIndex).val();
                         //alert(subject);
                        var pushNotification=$("#pushNotification"+childIndex).val();
                        console.log("pushNotification "+pushNotification);
                        var msgBody = $("#msgBody" + childIndex).summernote().code(); //.val();
                        //alert(msgBody);
                        var responseId = $("#" + "campaignMessageses" + childIndex + "\\.campaignMessagesResponses" + responseIndx + "\\.response\\.responseId").val();
                        //alert("responseId# " + responseId);
                        var intervalId = $("#" + "campaignMessageses" + childIndex + "\\.campaignMessagesResponses" + responseIndx + "\\.intervals\\.intervalId").val();
                        //alert("intervalId# " + intervalId);
                        var nextMessage = $("#" + "campaignMessageses" + childIndex + "\\.campaignMessagesResponses" + responseIndx + "\\.nextMessage").val();
                        // alert("nextMessage# " + nextMessage);
                        var repeatIntervalId = $("#" + "campaignMessageses" + childIndex + "\\.campaignMessagesResponses" + responseIndx + "\\.repeatIntervalId").val();
                        //alert("repeatIntervalId# " + repeatIntervalId);
                        var repeatMessageId = $("#" + "campaignMessageses" + childIndex + "\\.campaignMessagesResponses" + responseIndx + "\\.repeatMessageId").val();
                        //alert("repeatMessageId# " + repeatMessageId);
                        var messageId = $("#campaignMessageses" + childIndex + "\\.messageId").val();
                        // alert("messageId "+messageId);
                        var campaignMessagesResponseId = $("#" + "campaignMessageses" + childIndex + "\\.campaignMessagesResponses" + responseIndx + "\\.campaignMessagesResponseId").val();
                        var data = new FormData();
                        data.append("campaignId", $("#campaignId").val());
                        data.append("folderId", folderId);
                        data.append("selectedEvents", selectedEvents);
                        data.append("messageTypeId", messageTypeId);
                        data.append("isCritical", isCritical);
                        data.append("subject", subject);
                        data.append("msgBody", msgBody);
                        data.append("responseId", responseId);
                        data.append("intervalId", intervalId);
                        data.append("nextMessage", nextMessage);
                        data.append("repeatIntervalId", repeatIntervalId);
                        data.append("repeatMessageId", repeatMessageId);
                        data.append("fhCid", fhCid);
                        data.append("messageId", messageId);
                        data.append("campaignMessagesResponseId", campaignMessagesResponseId);
                        data.append("pushNotification",pushNotification);

                        $.ajax({
                            url: "${pageContext.request.contextPath}/campaign/saveCampaignMsgs",
                            type: "POST",
                            contentType: false,
                            ...
                            success: function (response) {
                                ...
                                // alert("statuscode "+obj.statuscode);
                                if (obj.statuscode == 200) {
                                    $("#message").html(obj.errorMessage);
